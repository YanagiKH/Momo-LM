...
安裝時會複製兩個帶逐張量完整性中繼資料的 NPZ checkpoint：
...
- 基礎文字 checkpoint 的直接自由生成仍可能重複、偏題或產生亂碼。文件檢索會把已匯入知識的命中片段與來源加入回應流程；目前沒有 A/B 評估證明它提升回答品質，也不能補足模型容量。
...
先把經常變動、需要來源的事實放入知識庫。若要嘗試以小批對話資料調整語氣與回應模式，請另外保留未參與訓練的行為評測；隨附評估不衡量這些效果：
...
- 3,963 個圖像參數適合驗證訓練與風格標籤 conditioning 的程式路徑；現有資料中每個 style label 只對應一張 image，不能隔離或證明風格控制效果，也不足以產生市場級動漫、漫畫、插畫或照片。
